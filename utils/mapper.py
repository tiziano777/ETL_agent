...
class Mapper:
    ...
    # ---------------- transform resolution ----------------
    def _resolve_transformation(self, transformation: Optional[Any], src_value: Any, sample_root: Any) -> Any:
        self._ensure_initialized()
        ...
        if src_value in (None, "N/A"):
            return transformation  # restituisci il valore fisso definito nel mapping
    ...
    def instantiate_from_schema_minimal(self, schema: Dict[str, Any]) -> Any:
        ...
        if "default" in schema:
            ...
            return d
        if "enum" in schema and isinstance(schema["enum"], list) and len(schema["enum"]) > 0:
            return schema["enum"][0]  # MODIFICA ENUM: usa valore valido di default
        ...
        if typ_primary == "object" or ("properties" in schema and typ_primary is None):
            ...
            for pname, pschema in props.items():
                ...
                if "default" in pschema:
                    ...
                    continue
                if "enum" in pschema and isinstance(pschema["enum"], list) and pschema["enum"]:
                    root_obj[pname] = pschema["enum"][0]  # MODIFICA ENUM: usa valore valido
                    continue
                ptype = pschema.get("type")
                if isinstance(ptype, list) and "null" in ptype:
                    root_obj[pname] = None
